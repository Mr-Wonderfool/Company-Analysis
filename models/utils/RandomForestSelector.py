...
class RandomForestSelector:
    ...
    def fit(self):
        ...
        )  # importance value of features, with higher score indicating higher importance
        self.indices = np.argsort(self.importances)[::-1]
        self.std = np.std(
            [tree.feature_importances_ for tree in dtree.estimators_], axis=0
        )
        return self.indices

    def plot(self, columns, figsize=(15, 6), threshold=0.2):
        ...
        ax.bar(
            X_indices,
            self.importances[self.indices][:filter_],
            width=0.5,
            color="b",
            yerr=self.std[self.indices][:filter_],
            align="center",
        )
        plt.xticks(X_indices, self.indices[:filter_])
        ax.set_xticklabels(
            columns[self.indices[:filter_]], rotation=30, fontsize="small", ha="right"
        )
        return fig, ax
