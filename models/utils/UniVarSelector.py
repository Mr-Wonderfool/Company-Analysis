...
class UniVarSelector:
    ...
    def fit(self):
        selector = SelectPercentile(
            f_classif if "f_classif" == self.method else mutual_info_classif,
            percentile=self.percent,
        )  # choose 20% of the variables
        selector.fit_transform(self.X, self.Y)
        self.selected_feautes = selector.get_support(indices=True)
        if self.method == "f_classif":
            pvalues = (
                selector.pvalues_
            )  # smaller values indicate the variable is more important
    ...
    def plot(self, columns, figsize=(15, 6), threshold=0.1):
        ...
        ax.bar(
            X_indices,
            self.scores[self.indx][:filter_],
            width=0.75,
            label=r"mutual information score",
            color="b",
        )
        ax.set_xticks(X_indices, self.indx[:filter_])
        ax.set_xticklabels(
            columns[self.indx[:filter_]], rotation=30, fontsize="small", ha="right"
        )
        plt.legend()
        return fig, ax
